...
cargo run --release -p klams-bench --bin run -- \
    --klams-url http://localhost:7777 \
    --klams-token "${KLAMS_TOKEN}" \
    --output specs/008-activity-observability/perf-baseline.md
```
...
| `--output`       | `specs/008-activity-observability/perf-baseline.md`     | Markdown file to write. |
...
**Filled example** (illustrative; real output is committed to `specs/008-activity-observability/perf-baseline.md`):
