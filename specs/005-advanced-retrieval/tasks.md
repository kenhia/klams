...
- [ ] T038 [US3] _Deferred — KnowledgeDigest qdrant writeback follows once stale-cluster detection lands. Event summaries cover the Phase 5 critical path._ Extend the digest path in `crates/klams-store/src/qdrant.rs`: write digests with `kind = "digest"` payload; retrieval filters `invalidated_at = null` and respects callers' raw-vs-digest preference.
- [ ] T039 [US3] _Deferred — SummaryStore.list_event_summaries available; substitution wiring inside ContextBuilder follows in a focused commit so the budget pass can be re-tested._ Wire summary substitution into `ContextBuilder` (`crates/klams-core/src/context.rs`): when raw items would blow a section's allowed budget, swap in the matching summary record (`source_count`, `source_ids`).
