...
- [ ] T055 _Deferred — SC-001/SC-003 latency benchmark requires a populated fixture (≥ 1 000 facts, ≥ 5 000 knowledge chunks, ≥ 10 000 events) plus a benchmark harness; tracked separately from this sprint's code drop._ Run the SC-001 / SC-003 benchmark on a populated fixture (≥ 1 000 facts, ≥ 5 000 knowledge chunks, ≥ 10 000 events). Record p95 latencies and budget-conformance numbers in `specs/005-advanced-retrieval/quickstart.md` "Validation" section. _(Fixture now available as of sprint 006: `FixtureScale::large()` in [`tests/scale_loader.rs`](../../tests/scale_loader.rs) provides ~10k facts / ~20k knowledge chunks / ~50k events behind the `scale-fixture` cargo feature. Bench harness still TBD — landing the bench is a follow-up sprint, not blocking sprint 006.)_
...
- [X] T058 _Deferred — end-to-end walkthrough needs a populated klams-service against live Postgres/Qdrant/Ollama; tracked alongside T055/T056._ Run end-to-end through `specs/005-advanced-retrieval/quickstart.md`. Tick each acceptance bullet. _(Quickstart §10 now maps each non-UI acceptance bullet to its deferred integration test — T016/T023/T024/T033/T043 — exercised against the live test stack. Viewport bullet remains tracked under T047.)_
