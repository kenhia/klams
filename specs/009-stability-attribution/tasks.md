...
**Input**: Design documents from `specs/009-stability-attribution/`
...
- [X] T013a [US1] 18h soak complete 2026-05-31; 259 201/259 201 requests, 0 failures, max CLOSE_WAIT = 0 across all 2161 samples. SC-001 PASS. Verdict recorded in [specs/009-stability-attribution/soak-report.md](specs/009-stability-attribution/soak-report.md).
...
- [X] T044 [P] Move sprint 009 backlog rows from [specs/planning/backlog.md](specs/planning/backlog.md) to [specs/planning/backlog-archive.md](specs/planning/backlog-archive.md) (the entries covering #26, #28, attribution, repair, perf-rerun, test-isolation)
- [X] T045 Run `just gate` and confirm fmt / clippy / tests all green at HEAD of `009-stability-attribution`
- [X] T046 Walk [quickstart.md](specs/009-stability-attribution/quickstart.md) end-to-end on `kubs0`; check each step. While walking, record the SC-003 verdict (sample share of `system`-attributed REST writes from a 200-row sample), the SC-004 verdict (per-author counts before and after `reattribute-system --apply`, including the `lost-author` bucket), and the SC-005 verdict (first-click results for a 20-row sample across multiple authors) into [specs/008-activity-observability/perf-baseline.md](specs/008-activity-observability/perf-baseline.md) under a new "Sprint 009 acceptance" section
