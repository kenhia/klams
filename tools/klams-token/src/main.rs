...
impl Session {
    ...
    fn commit(
        ...
        // The manifest describes the config being REPLACED — `before`
        // is its grant set — so an encrypted backup stays legible to
        // krot and to an audit without anyone decrypting it (#1384).
        // Sprint 049: identities are listed alongside the token grants,
        // with `-` where a token digest would be. A manifest that
        // silently omitted them would understate what the backup holds.
        let durable = writer::DurableBackup {
            recipient: self.recipient.as_ref().map(|r| r.value.as_str()),
            fingerprints: before
                .tokens
                .iter()
                .chain(&before.identities)
                .map(|f| (f.key.clone(), f.token.clone()))
                .collect(),
        };
        let written = writer::write_validated(
