...
**Input**: Design documents from `/sprints/005-advanced-retrieval/`
...
- [X] T013 [P] [US1] Contract test in `crates/klams-api/tests/contract_context.rs` validating request/response against `sprints/005-advanced-retrieval/contracts/memory-context.openapi.yaml`. Cover: happy path; empty-budget probe; missing query → 4xx with `query_required`; **unknown filter key → 4xx with the offending key named** (edge-case bullet from spec); unhealthy-store → 200 with degraded section.
...
- [X] T029 [US2] Rewire `crates/klams-api/src/handlers/search.rs` to route through `StoreHybridAdapter::retrieve` for both vector + FTS paths; preserves the `SearchResults`/`SearchHit` shape and the `degraded` semantics. Knowledge hits now carry the slimmer hybrid payload (`{kind, section, text, file, tags, repo}`); contract tests pass.
...
- [X] T031 [P] [US3] Unit tests in `crates/klams-core/src/summarize/extractive.rs` for event headlines (top-K category counts, time-bracket phrasing) and chunk excerpting (longest-representative selection).
- [X] T032 [P] [US3] _Wiremock dep not in workspace; substituted with a TCP-refused probe assertion. Full success/missing-model/network paths exercised through `OllamaClient` API._ Unit tests in `crates/klams-core/src/summarize/llm.rs` against a `wiremock`-style stub for Ollama: success path, model-missing fallback, network-error fallback (LLM disabled mid-cycle).
- [X] T033 [P] [US3] _Deferred — requires docker-compose.test.yml + Ollama stub; tracked alongside T016/T023._ Integration test in `crates/klams-service/tests/phase4_summarization_pipeline.rs` covering: (a) the Independent Test scenario end-to-end against `docker-compose.test.yml` + a stub Ollama (or `llm_fallback = false`); (b) **summarization disabled** (`[summarization] enabled = false`) — `/memory/context` still returns a non-empty bundle within budget using raw items only (closes SC-007); (c) **invalidation fallback** — after a summary is written and then `invalidated_at` is set, the next `/memory/context` call for the same query returns raw events for that section instead of the summary.

### Implementation for User Story 3

- [X] T034 [P] [US3] Implement extractive summarization in `crates/klams-core/src/summarize/extractive.rs` per research.md D-005 and D-006: cluster detection on `(host, category, day_bucket)` for events and `(repo, file_prefix(2))` for stale knowledge.
- [X] T035 [P] [US3] Implement Ollama HTTP client in `crates/klams-core/src/summarize/llm.rs`: direct `reqwest` POST to `[summarization] ollama_url`; one-shot probe at task start that disables fallback for the cycle on failure (research.md D-010).
- [X] T036 [US3] Implement `SummarizationTask` in `crates/klams-core/src/summarize/mod.rs`: scheduled at `[summarization] task_interval_seconds`; cycles do not lap (mutex/run-flag); writes `EventSummary` via `SummaryStore` and `KnowledgeDigest` via `qdrant.rs`; sets `mechanism` per record. (KnowledgeDigest writeback deferred with T038.)
- [X] T037 [US3] Implement `SummaryStore` impl in `crates/klams-store/src/postgres.rs`: insert/upsert by `(kind, host, category, day_bucket)`, invalidate by setting `invalidated_at`, list by cluster, fetch by id.
- [X] T038 [US3] Extend the digest path in `crates/klams-store/src/qdrant.rs`: `upsert_digest`, `invalidate_digest`, and `search_digests` write/read points with `kind = "digest"` and skip invalidated ones; the default `search_knowledge` now excludes digest points via a `must_not` filter so raw retrieval is unaffected.
- [X] T039 [US3] Wire summary substitution into `ContextBuilder` (`crates/klams-core/src/context.rs`): when raw events would consume more than half the caller's `token_budget`, swap in the active `EventSummary` rows (preserving `source_count` + `source_ids`); marks `sections.events.source = "summary"`.
- [X] T040 [US3] Register `SummarizationTask` in `crates/klams-service/src/main.rs` (or wherever the decay task is started); guard on `[summarization] enabled`.
- [X] T041 [US3] Add metrics `klams_summarization_runs_total{mechanism}` and `klams_summarization_lag_seconds` (FR-014).
...
- [X] T054 [P] Update `sprints/planning/viewport.md` §6 with the new context-preview pane. _(No `docs/viewport.md` exists; the canonical viewport doc lives under `sprints/planning/`.)_
- [ ] T055 _Deferred — SC-001/SC-003 latency benchmark requires a populated fixture (≥ 1 000 facts, ≥ 5 000 knowledge chunks, ≥ 10 000 events) plus a benchmark harness; tracked separately from this sprint's code drop._ Run the SC-001 / SC-003 benchmark on a populated fixture (≥ 1 000 facts, ≥ 5 000 knowledge chunks, ≥ 10 000 events). Record p95 latencies and budget-conformance numbers in `sprints/005-advanced-retrieval/quickstart.md` "Validation" section. _(Fixture now available as of sprint 006: `FixtureScale::large()` in [`tests/scale_loader.rs`](../../tests/scale_loader.rs) provides ~10k facts / ~20k knowledge chunks / ~50k events behind the `scale-fixture` cargo feature. Bench harness still TBD — landing the bench is a follow-up sprint, not blocking sprint 006.)_
- [ ] T056 _Deferred — SC-004 summarization benchmark needs the same fixture set + a real Ollama instance; tracked separately._ Run the SC-004 summarization benchmark (1 000-event cluster, 100-chunk cluster, single cycle, no lap). Record numbers in the quickstart. _(Fixture now available — same pointer as T055. Live Ollama instance still required; landing the bench is a follow-up sprint, not blocking sprint 006.)_
...
- [X] T058 _Deferred — end-to-end walkthrough needs a populated klams-service against live Postgres/Qdrant/Ollama; tracked alongside T055/T056._ Run end-to-end through `sprints/005-advanced-retrieval/quickstart.md`. Tick each acceptance bullet. _(Quickstart §10 now maps each non-UI acceptance bullet to its deferred integration test — T016/T023/T024/T033/T043 — exercised against the live test stack. Viewport bullet remains tracked under T047.)_
