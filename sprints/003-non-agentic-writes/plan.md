...
**Input**: Feature specification from `/sprints/003-non-agentic-writes/spec.md`
...
5. The handoff document for ansible-k is staged under
   `sprints/003-non-agentic-writes/handoff/` during the sprint and
...
|-----------|------|--------|
| I. SDD | Spec exists at `sprints/003-non-agentic-writes/spec.md`, all FRs traceable to user stories. | PASS |
...
```text
sprints/003-non-agentic-writes/
|-- plan.md              # This file
...
- [quickstart.md](quickstart.md) — operator-facing walkthrough: build → run migration → `just install-systemd` → `journalctl -u klams-service` smoke → scanner timer first-fire → monitor service-restart test → `curl GET /memory/policy` → `curl POST /memory/facts` with a `source=Task` payload showing `path: "canonical"` → `cp -r sprints/003-non-agentic-writes/handoff/ /home/ken/ansible-k/specs/klams-integration/` → re-run sprint 002 walkthrough to confirm zero regressions.

### Agent context update

The reference between the `<!-- SPECKIT START -->` and `<!-- SPECKIT END -->` markers in `.github/copilot-instructions.md` will be updated to point at `sprints/003-non-agentic-writes/plan.md` (currently at sprint-002).
