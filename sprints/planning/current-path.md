...
`sprints/00X-*/` spec→plan→tasks→implement discipline and the `speckit.*` agents.
...
2. **Migrations belong at sprint boundaries.** klams has nine sprints of Spec
   Kit artifacts under `sprints/`. Changing the planning substrate is structural
...
3. **It deserves its own evaluation.** ATV touches `.github/`, `.vscode/`,
   ...
   happens to the existing `sprints/` tree, the `.specify/` constitution, and
...
- **Audit what it clobbers.** Confirm how the installer treats the existing
  `.github/copilot-instructions.md`, `.specify/`, and `sprints/`. ATV's
  uninstall claims checksum-based preservation of user-modified files and never
  touches `.vscode/`; verify that against our tree.
- **Decide the `sprints/` fate explicitly.** Keep the historical Spec Kit specs
