...
| `test-integration`| The docker-gated suite `gate` excludes. Sweeps the test stack (`scripts/reset-test-stack.sh`), then runs `cargo test --workspace -- --ignored` at default parallelism. Needs `docker compose -f tests/docker-compose.test.yml up -d --wait`; run `... down` when finished. The sweep waits up to `TEST_STACK_WAIT_SECS` (default 60) for qdrant and postgres to be *ready* rather than merely started (#2283), so a bare `up -d` works too — `--wait` additionally covers TEI and the reranker — a long-lived test stack shadows the production containers and accumulates seeds (#647). |
...
| `publish`         | Sprint 042 (#1012): build all four binaries and publish them to the homelab package store under `artifacts/klams-{service,scanner,monitor,token}/<version>/`. Refuses a dirty tree, and refuses binaries that disagree about their version. Needs `KLAMS_STORE_HOST`. `klams-token` joined the set in sprint 048 (#1697) — it has no unit, so `/healthz` cannot report it drifting. |
| `deploy-from-store *ARGS` | Install binaries on **this** host from the store, SHA256-verified. Defaults to all four at `latest`; name specific binaries, or `--version <v>` to roll back. Restarts nothing. Needs `KLAMS_STORE_URL`. |
