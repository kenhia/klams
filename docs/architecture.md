...
*(The SVGs under [`docs/diagrams/`](diagrams/) are hand-authored,
sprint 033 #694 — theme-aware, self-contained, and drawn strictly from
what exists; navigate the codebase by the names in the boxes. That
directory also holds one **generated** artifact — see
[`docs/diagrams/README.md`](diagrams/README.md) for which is which, and
how the generated one is reproduced.)*

**[▶ Interactive component map](https://htmlpreview.github.io/?https://github.com/kenhia/klams/blob/main/docs/diagrams/klams-architecture.html)**
— the same topology as an explorable page: guided views, search,
relationship tracing, light/dark, no external requests. Generated by
archify from [`klams.architecture.json`](diagrams/klams.architecture.json)
and pinned to the revision it describes. It shows structure only; the
behaviour is the rest of this document.
